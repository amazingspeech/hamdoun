...
def test_vervang_product_verkoopdata_en_teruglezen(tmp_path):
    ...
    assert haal_product_verkoopdata(engine, organisatie_id=org_id) == [
...
def test_vervang_product_verkoopdata_vervangt_volledig(tmp_path):
    ...
    vervang_product_verkoopdata(engine, organisatie_id=org_id, rijen=[("2026-02-01", "Melk", 3)])
...
def test_product_verkoopdata_geeft_gesorteerd_op_datum_terug(tmp_path):
    ...
    datums = [r["datum"] for r in haal_product_verkoopdata(engine, organisatie_id=org_id)]
...
def test_product_verkoopdata_is_geisoleerd_per_organisatie(tmp_path):
    engine = maak_database(tmp_path / "tenants.db")
    ...
    vervang_product_verkoopdata(engine, organisatie_id=org_a, rijen=[("2026-01-01", "Brood", 10)])

    assert haal_product_verkoopdata(engine, organisatie_id=org_b) == []
