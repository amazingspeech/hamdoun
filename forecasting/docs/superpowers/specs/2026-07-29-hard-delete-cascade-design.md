...
- **De audit-log** (`security/audit.py`). Eén gedeeld, append-only logbestand voor alle organisaties samen, bevat alleen metadata (key-naam, store_id, statuscode, latency) — geen wachtwoorden, keys of andere directe persoonsgegevens. Technisch onpraktisch om één organisatie er middenin uit te knippen zonder het hele bestand te herschrijven, en gangbaar om beveiligingslogs een eigen, kortere bewaartermijn te geven los van de verwijderplicht op klantdata zelf.
